...
cv::Mat vonKries(const cv::Mat& input)
{
    ...
    // 使用 CUDA 進行 von Kries 轉換
    cv::Mat XYZ_vonKries = XYZ.clone();
    launchVonKriesKernel(XYZ_vonKries, K_xyz);
    auto end = std::chrono::high_resolution_clock::now();
    std::cout << "von kries Time: " << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << " ms" << std::endl;
